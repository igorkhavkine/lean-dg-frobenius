...
[[The idea now is to rewrite the proof of Lem.~\ref{lem:eq-radial} to just refer
to a more abstract result (local existence of "parallel sections with respect to a connection")
to be formulated in Lem.~\ref{lem:eq-parallel-section}, whose proof will repeat the logic
of the current proof of Lem.~\ref{lem:eq-radial} in a bit more generality.]]
...
$[0,1]\times U'$:
\begin{equation}
  \frac{\partial}{\partial y^a} \zeta^c(t,y,z) - t g_a^c(ty, \zeta(t,y,z)) = 0 .
\end{equation}
\end{lemma}
...
The remaining implication about the solution of~\eqref{eq:radial-parallel-section} follows
...
(b) [[adapt the argument currently in Lem.~\ref{lem:eq-radial}]]

%%% ignore the rest here
%In the simple direction, if local solutions exist on $U$, then differentiating the
%equation~\eqref{eq:parallel-section} with respect to $x^{a'}$ and anti-symmetrizing
%($a \leftrightarrow a'$) annihilates the left-hand side, while the eliminating
%$\frac{\partial}{\partial x^{a'}} v^c(y,x)$ using~\eqref{eq:parallel-section} itself directly
%gives the desired identity~\eqref{eq:zero-curvature}.
%
%In the resverse direction, we need to define a candidate local solution family
%$w^c(y,x,z)$ and verify that it satisfies the desired conditions (a--d)
%provided the zero-curvature condition (e) $R_{a'a}^c(y,x,z) = 0$ holds.
%
%Let us define the following family of ODEs parametrized by $(y,z) \in U'
%\subseteq B \times F$, obtained by contracting both sides
%of~\eqref{eq:parallel-section} with $y^a$ while replacing $y^a
%\frac{\partial}{\partial x^a}$ with $\frac{\partial}{\partial t}$ and $v^c(x)$
%with $\zeta^c(t,y,z)$:
%\begin{equation} \label{eq:radial-parallel-section}
%  \frac{\partial}{\partial t} \zeta^c(t,y,z) = y^a g_a^c(t y, \zeta(t,y,z)) .
%\end{equation}
%let $\zeta^c(t,y,z)$ be the solution of the initial above value problem with
%$\zeta^c(0,y,z) = z^c$, which by Lemma~\ref{lem:eq-radial-exists} exists on some
%neighborhood of $[-1,1] \times U$ of $(0,0,0)$ for some sub-neighborhood $U
%\subseteq U'$, is unique and jointly smooth in all variables. Then the candidate
%local solution is defined by $w^c(y,z) = \zeta^c(1,y,z)$, which is obviously (c)
%jointly smooth and by construction satisfies (d) $w^c(0,z) = \zeta^c(1,0,z) =
%z^c$.
%
%Now we need to check that $w^c(y,z)$ indeed solves~\eqref{eq:parallel-section}.
%Incidentally, if the desired local solution family exists, then the
%corresponding solution to the ODE initial value problem is given by
%$\zeta^c(t,y,z) = w^c(ty,z)$.
